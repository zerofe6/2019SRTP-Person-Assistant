...
public class LoginHttpServlet extends HttpServlet {
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		// 拼接SQL查询语句
		String sql = "select * from userinfo where account = "+"'"+requestParam.getString("name")+"'";
		System.out.println("登录的sql语句为："+sql);
		...
		try {
			...
			if (result.next()) {
				if (result.getString("password").equals(requestParam.getString("password"))) {
					...
					res.getProperty().put("username", result.getString("account"));
				} else {
